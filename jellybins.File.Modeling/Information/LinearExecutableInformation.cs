...
public class LinearExecutableInformation : IExecutableInformation
{
    ...
    {
        byte cpu = Convert.ToByte(flag);
        return cpu switch
        {
            (byte)Processor.I286Instructions => "Intel x86 (i286)",
            (byte)Processor.I386Instructions => "Intel x86 (i386)",
            (byte)Processor.I486Instructions => "Intel x86 (i486)",
            _ => string.Empty
        };
    }
    ...
    {
        byte os = Convert.ToByte(flag);
        return os switch
        {
            (byte)OperatingSystem.OS2 => "IBM OS/2",
            (byte)OperatingSystem.Windows => "Microsoft Windows",
            (byte)OperatingSystem.Windows386 => "Microsoft Windows/386",
            (byte)OperatingSystem.DOS => "Microsoft DOS",
            _ => string.Empty
        };
    }
