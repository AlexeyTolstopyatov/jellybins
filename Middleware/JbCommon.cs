...
{
    internal static class JbCommon
    {
        /// <summary>
        /// Записывает информацию о функциях, использованных
        /// в исследуемом двоичном файле
        /// </summary>
        /// <param name="assembly"></param>
        /// <param name="netp"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Заполняет страницу отчета распознанными данными из
        /// Структуры PE-заголовка общего вида
        /// </summary>
            ...
            ref NtHeader nt)
        {
            ...
            foreach (string characteristic in JbPeInformation.CharacteristicsToStrings(nt.WinNtMain.Characteristics))
            {
            ...
            JbAppReport.Information("Saving sources...");
            foreach (System.Windows.Controls.TextBlock item in bin.FlagsNames.Items)  bin.FlagNamesSource.Add(item.Text);
            foreach (System.Windows.Controls.TextBlock item in bin.LoaderFlags.Items) bin.FlagsSource.Add(item.Text);
            foreach (CardExpander list in bin.bintable.Children)
            {
                JbStringHead stringHead = new($"{list.Header}");
                foreach (string item in (list.Content as ListView)!.Items)
                {
                    stringHead.List.Add(item);
                }
            }
        ...
        /// <summary>
        /// Записывает информацию, распознанную из NE заголовка
        /// в страницу общего отчета
        /// </summary>
        /// <param name="bin"></param>
        /// <param name="ne"></param>
        /// <returns></returns>
            ...
            ref NeHeader ne)
        {
            ...
            // Перечисление флагов загрузчика
            JbElement.SetFlagDescription("Основные", Brushes.Cyan, ref bin);
        ...
        /// <summary>
        /// Заполняет страницу отчета рспознанной информацией о LE исполняемом файле
        /// </summary>
        /// <param name="bin"></param>
        /// <param name="le"></param>
        /// <returns></returns>
        public static JbFileChars TryParseLeHeader(
            ...
            ref LeHeader le)
        {
            bin.binprops.Text = JbTypeInformation.GetInformation(JbFileType.Linear);
            bin.bintype.Text = JbTypeInformation.GetType(JbFileType.Linear);
            bin.bintable.Children.Add(
                new CardExpander()
                {
                    IsExpanded = true,
            ...
            bin.OsRequiredLabel.Text = JbLeInformation.OperatingSystemFlagToString(le.TargetOperatingSystem);
            bin.ArchRequiredLabel.Text = JbLeInformation.ProcessorFlagToString(le.CPUType);

            JbElement.SetFlagDescription("Флаги определения модуля", Brushes.Cyan, ref bin);
            foreach (var mtFlag in JbLeInformation.ModuleFlagsToStrings(le.ModuleTypeFlags))
            {
                if (mtFlag != string.Empty)
                    JbElement.SetFlag(ref bin.LoaderFlags, mtFlag, Brushes.Cyan);
            }
            
            // Я не смог найти информацию о номерах виртуальных устройств,
            // но полагаю, версия Device Driver Kit точно у Microsoft 0 быть не может
            // Если конечно это не пре-релиз.......
            if (le.WindowsDDKVersion != 0)
            {
                JbElement.SetFlagDescription("Флаги определения для Windows", Brushes.CornflowerBlue, ref bin);
                ...
                TryParseVxdHeader(ref bin, ref le);
        ...
        /// <summary>
        /// Записывает распознанную информацию из CLR заголовка
        /// в страницу общего отчета
        /// </summary>
        /// <param name="bin"></param>
        ...
        /// <summary>
        /// Заполняет страницу отчета распознанной информацией о LE+VDD подписанном драйвере
        /// устройства.
        /// </summary>
        /// <param name="bin"></param>
        ...
        /// <summary>
        /// Распознает данные о расположении секций
        /// </summary>
        /// <param name="bin"></param>
        /// <param name="nt"></param>
        private static void TryParseDataTable(
            ref BinaryHeaderPage bin,
            ref NtHeader nt)
        {
        ...
        /// <summary>
        /// Записывает распознанную информацию из MZ заголовка
        /// в страницу отчета
        /// </summary>
        ...
        /// <summary>
        /// Записывает в странице информацию о файле
        /// операционной системы или другом файле-образце, который
        /// указывает требования к его запуску.
        ///
        /// Полагаю, что это единственный способ не писать большую кодовую базу
        /// с использованием Windows API собственноручно чтобы узнать данные о архитектуре процессора
        /// </summary>
        /// <param name="bin"></param>
        /// <param name="nt"></param>
        /// <returns></returns>
